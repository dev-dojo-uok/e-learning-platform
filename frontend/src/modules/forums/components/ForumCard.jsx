 ...
 *
 * Props:
 *   forum  – { id, name, description, moduleId?, module? }
...
export default function ForumCard({ forum, to }) {
  ...
  return (
    <Link
      to={href}
      className="group flex items-start gap-4 p-5 rounded-xl border border-border bg-card hover:bg-muted/30 transition-all duration-200 shadow-sm"
    >
      {/* Icon */}
      <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center bg-muted text-primary">
        {isModuleForum ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isModuleForum && forum.module?.title && (
            <span className="text-[10px] font-semibold bg-secondary text-secondary-foreground px-2 py-0.5 rounded-full whitespace-nowrap">
              {forum.module.title}
            </span>
          )}
        </div>
